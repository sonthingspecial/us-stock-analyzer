  ...
  return (
    <a
      ...
      className={`block bg-white dark:bg-gray-900 rounded-xl p-5 shadow-sm hover:shadow-md transition-shadow cursor-pointer
        ${isUrgent ? 'border-2 border-red-400 dark:border-red-500' : 'border border-gray-200 dark:border-gray-800'}`}
    >
    ...
    <>
      ...
      <main className="max-w-7xl mx-auto px-4 py-6">
        ...
        <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl p-4 mb-6 space-y-4">
          <div>
            <label className="text-xs font-semibold text-gray-600 dark:text-gray-300 mb-1.5 block uppercase tracking-wide">지역</label>
            <select
              value={region}
            ...
            {financeOpen && (
              <div className="px-4 py-4 space-y-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label className="text-xs text-gray-500 dark:text-gray-400 mb-1 block">보유 자본금 (만원)</label>
                    <input type="number" value={capital} onChange={(e) => setCapital(e.target.value)} placeholder="예: 5000"
                  ...
                  <div>
                    <label className="text-xs text-gray-500 dark:text-gray-400 mb-1 block">예상 대출가능액 (만원)</label>
                    <input type="number" value={loan} onChange={(e) => setLoan(e.target.value)} placeholder="예: 30000"
                      className="w-full bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg px-3 py-2 text-sm text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500" />
                  </div>
                </div>
                <div>
                  <label className="text-xs text-gray-500 dark:text-gray-400 mb-1.5 block">계약금 비율</label>
                  <div className="flex gap-2">
                    {CONTRACT_RATES.map((rate) => (
                      <button key={rate} onClick={() => setContractRate(rate)}
                        className={`px-4 py-1.5 rounded-lg text-sm font-medium transition-colors ${contractRate === rate ? 'bg-blue-600 text-white' : 'bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700'}`}>
                        {rate}%
                      </button>
                    ))}
                  </div>
                ...
                {affordablePrice > 0 && (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-1">
                    <div className="bg-blue-50 dark:bg-blue-900/20 rounded-lg px-4 py-3">
                      ...
                      <p className="text-lg font-bold text-blue-700 dark:text-blue-300">{affordablePrice.toLocaleString()}만원</p>
                    </div>
                    <div className="bg-orange-50 dark:bg-orange-900/20 rounded-lg px-4 py-3">
                      <p className="text-xs text-orange-600 dark:text-orange-400 mb-0.5">계약금 필요 금액 ({contractRate}%)</p>
                      <p className="text-lg font-bold text-orange-700 dark:text-orange-300">{downPaymentNeeded.toLocaleString()}만원</p>
                    </div>
                  </div>
                )}
              </div>
          ...
          <div>
            <label className="text-xs font-semibold text-gray-600 dark:text-gray-300 mb-1.5 block uppercase tracking-wide">청약 유형</label>
            <div className="flex flex-wrap gap-2">
              {APT_TYPES.map((t) => (
                <button key={t} onClick={() => toggleType(t)}
                  className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${selectedTypes.includes(t) ? 'bg-blue-600 text-white' : 'bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700'}`}>
                  {t}
                </button>
              ))}
            </div>
          ...
          <div>
            <label className="text-xs font-semibold text-gray-600 dark:text-gray-300 mb-1.5 block uppercase tracking-wide">브랜드</label>
            <div className="flex flex-wrap gap-2">
              {BRANDS.map((b) => (
                <button key={b} onClick={() => toggleBrand(b)}
                  className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${selectedBrands.includes(b) ? 'bg-indigo-600 text-white' : 'bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700'}`}>
                  {b}
                </button>
              ))}
            </div>
          ...
          <div>
            <label className="text-xs font-semibold text-gray-600 dark:text-gray-300 mb-1.5 block uppercase tracking-wide">청약 마감 일정</label>
            <div className="flex flex-wrap gap-2">
              <button onClick={() => setScheduleWeeks(null)}
                className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${scheduleWeeks === null ? 'bg-gray-700 dark:bg-gray-200 text-white dark:text-gray-900' : 'bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700'}`}>
                전체
              </button>
              {SCHEDULE_OPTIONS.map(({ label, weeks }) => (
                <button key={weeks} onClick={() => setScheduleWeeks(weeks)}
                  className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${scheduleWeeks === weeks ? 'bg-gray-700 dark:bg-gray-200 text-white dark:text-gray-900' : 'bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700'}`}>
                  {label}
                </button>
              ))}
            </div>
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <p className="text-sm text-gray-400 dark:text-gray-500 mt-1">필터 조건을 변경하거나 전체 기간으로 조회해 보세요</p>
          </div>
        ) : (
          <>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {items.map((item) => <AptCard key={item.id} item={item} />)}
            </div>
          </>
        )}
        ...
        <footer className="border-t border-gray-200 dark:border-gray-800 mt-10 pt-6 text-center">
          ...
          <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">
            ...
            <a href="https://www.applyhome.co.kr" target="_blank" rel="noopener noreferrer"
              className="underline hover:text-gray-600 dark:hover:text-gray-300">청약홈</a>
            에서 확인하세요
          </p>
        </footer>
      </main>
